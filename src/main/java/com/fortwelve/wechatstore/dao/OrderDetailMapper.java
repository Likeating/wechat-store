...
public interface OrderDetailMapper {
    @Insert("insert into order_detail (order_id,sku_id,product_name,sku_attr,sku_price,num) values (#{order_id},#{sku_id},#{product_name},#{sku_attr},#{sku_price},#{num})")
    @Options(useGeneratedKeys = true,keyProperty = "id",keyColumn = "id")
    public int addOrderDetail(OrderDetail orderDetail);

    @Update("update order_detail set order_id=#{order_id},sku_id=#{sku_id},product_name=#{product_name},sku_attr=#{sku_attr},sku_price=#{sku_price},num=#{num} where id=#{id}")
